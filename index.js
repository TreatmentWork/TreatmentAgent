...
function NodeJSScanner(options) {
    ...
    if (!__.isEmpty(this.settings.quarantine_infected) && !fs.existsSync(this.settings.quarantine_infected)) {
        ...
        if (this.settings.debug_mode)
            console.log("Nodejs-ClamAv-Scanner: " + err_msg);
    }
...
NodeJSScanner.prototype.scan_files = function(files, end_cb, file_cb) {
    ...
    // The function that parses the stdout from clamscan
    ...
    var do_scan = function(files) {
        ...
        // The MUCH quicker but less-verbose way
            ...
            var finish_scan = function() {
                ...
                if(self.settings.debug_mode === true)
                    console.log('Nodejs-ClamAv-Scanner: Configured clam command: ' + self.settings[self.scanner].path + ' ' +self.build_clam_args(items).join(' '));

                // Execute the clam binary with the proper flags
                execFile(self.settings[self.scanner].path, self.build_clam_args(items), function(err, stdout, stderr) {
                    if(self.settings.debug_mode === true) {
                        console.log('Nodejs-ClamAv-Scanner: stdout:', stdout);
                    }
                    if (err && stderr) {
                        if(self.settings.debug_mode === true){
                            console.log('Nodejs-ClamAv-Scanner: An error occurred.');
                            console.error(err);
                            console.log('Nodejs-ClamAv-Scanner: ' + stderr);
                        }
